...
export async function smsHandler(notification: {
  id: string;
  subject: string | null;
  body: string | null;
  recipientId: string;
  ...
  await sleep(200); // simulate Twilio latency
  ...
  return {
    success: true,
    providerMessageId: `stub_twilio_${crypto.randomUUID()}`,
    error: null,
  };
}
